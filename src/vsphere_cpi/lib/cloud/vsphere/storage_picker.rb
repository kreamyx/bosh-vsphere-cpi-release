module VSphereCloud
  module StoragePicker
    ...
    def choose_persistent_storage(size, disk_pool, available_datastores)
      ...
      if datastore_names.empty?
        ...
        target_persistent_pattern = disk_pool.datacenter.persistent_pattern
      else
        @logger.info("Using datastore list: #{datastore_names.join(', ')}")
        target_persistent_pattern = "^(#{datastore_names.map do |name|
          Regexp.escape(name)
        end.join('|')})$"
      end
      ...
      disk_pool.datacenter.find_datastore(datastore_name)
    end
  end
end